	...
	if (! raw__primobjectp(exp, cause)) {
	  ...
	  if (f2primobject__is_cons(exp, cause)) {
	    ...
	    {f2__write_pretty__slot_key_and_value("left_concept",    15, cause, stream, f2conceptnet_relation__left_concept(exp, cause), f2conceptnet_relation__left_concept__tracing_on(exp, cause), f2conceptnet_relation__left_concept__trace(exp, cause), f2conceptnet_relation__left_concept__imagination_frame(exp, cause),
						  ((recursion_depth == -1) ? recursion_depth : (recursion_depth - 1)), indent_space_num, available_width - width, subexp_size, try_wide, wide_success, show_slot_causes, use_ansi_colors, use_html); width += subexp_size[0]; height += subexp_size[1];}
	    if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
	    {f2__write_pretty__slot_key_and_value("right_concept", 15, cause, stream, f2conceptnet_relation__right_concept(exp, cause), f2conceptnet_relation__right_concept__tracing_on(exp, cause), f2conceptnet_relation__right_concept__trace(exp, cause), f2conceptnet_relation__right_concept__imagination_frame(exp, cause),
						  ((recursion_depth == -1) ? recursion_depth : (recursion_depth - 1)), indent_space_num, available_width - width, subexp_size, try_wide, wide_success, show_slot_causes, use_ansi_colors, use_html); width += subexp_size[0]; height += subexp_size[1];}
	  } else {
