...
int f2__thread__bytecode_helper__jump_funk__no_increment_pc_reg(f2ptr thread, f2ptr cause) {
  ...
  if(raw__cfunkp(funktion, cause)) {
    ...
    return 1;
  } else if (raw__funkp(funktion, cause)) {
    ...
    if (raw__exceptionp(body_bcs, cause)) {
...
int f2__thread__bytecode__compile__tracewrap__protect_environment(f2ptr thread) {
  ...
  f2thread__value__set(thread, cause, raw__compile(cause, (tracewrap != nil), thread, f2thread__value(thread, cause), (protect_environment != nil), (protect_environment == nil), NULL, &is_funktional, nil));
  return 0;
}
