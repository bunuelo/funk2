...
void funk2_bytecode__init(funk2_bytecode_t* this) {
  ...
  this->bytecode__jump_machine_code__symbol                   = -1;
  this->bytecode__jump_machine_code__execution_count          = 0;
  this->bytecode__funk_env__symbol                            = -1;
  this->bytecode__funk_env__execution_count                   = 0;
  this->bytecode__funk_local_copy__symbol                     = -1;
  this->bytecode__funk_local_copy__execution_count            = 0;
...
void funk2_bytecode__destroy(funk2_bytecode_t* this) {
  ...
  status("  bytecode__funk_local_copy__execution_count            = " u64__fstr, this->bytecode__funk_local_copy__execution_count);
  ...
  status("  bytecode__jump_machine_code__execution_count          = " u64__fstr, this->bytecode__jump_machine_code__execution_count);
...
u64 raw__x86_funk_variable_type__byte_length(f2ptr cause, f2ptr expression) {
  if      (raw__eq(cause, expression, new__symbol(cause, "u64")))    {return sizeof(u64);}
  else if (raw__eq(cause, expression, new__symbol(cause, "s64")))    {return sizeof(s64);}
  else if (raw__eq(cause, expression, new__symbol(cause, "u32")))    {return sizeof(u32);}
  else if (raw__eq(cause, expression, new__symbol(cause, "s32")))    {return sizeof(s32);}
  else if (raw__eq(cause, expression, new__symbol(cause, "u16")))    {return sizeof(u16);}
  else if (raw__eq(cause, expression, new__symbol(cause, "s16")))    {return sizeof(s16);}
  else if (raw__eq(cause, expression, new__symbol(cause, "u8")))     {return sizeof(u8);}
  else if (raw__eq(cause, expression, new__symbol(cause, "s8")))     {return sizeof(s8);}
  else if (raw__eq(cause, expression, new__symbol(cause, "double"))) {return sizeof(double);}
  else if (raw__eq(cause, expression, new__symbol(cause, "float")))  {return sizeof(float);}
  else if (raw__eq(cause, expression, new__symbol(cause, "f2ptr")))  {return sizeof(f2ptr);}
...
int f2__fiber__bytecode__jump_machine_code(f2ptr fiber, f2ptr bytecode) {
  bytecode_status("bytecode jump_machine_code beginning.");
  f2ptr cause = f2fiber__cause_reg(fiber, nil);
  __funk2.bytecode.bytecode__jump_machine_code__execution_count ++;
  
  f2__fiber__increment_pc(fiber, cause);
  
  return f2__fiber__bytecode__jump_machine_code__no_increment_pc_reg(cause, fiber, bytecode);
}
...
int f2__fiber__bytecode__funk_local_copy(f2ptr fiber, f2ptr bytecode) {
  bytecode_status("bytecode funk_local_copy beginning.");
  f2ptr cause = f2fiber__cause_reg(fiber, nil);
  __funk2.bytecode.bytecode__funk_local_copy__execution_count ++;
  
  f2__fiber__increment_pc(fiber, cause);
  
  return f2__fiber__bytecode__funk_local_copy__no_increment_pc_reg(cause, fiber, bytecode);
}
...
void f2__bytecodes__reinitialize_globalvars() {
  ...
  __funk2.bytecode.bytecode__jump_machine_code__symbol          = new__symbol(cause, "jump_machine_code");
  __funk2.bytecode.bytecode__funk_local_copy__symbol            = new__symbol(cause, "funk_local_copy");
...
void f2__bytecodes__defragment__fix_pointers() {
  ...
  defragment__fix_pointer(__funk2.bytecode.bytecode__jump_machine_code__symbol);
  defragment__fix_pointer(__funk2.bytecode.bytecode__funk_local_copy__symbol);
