...
f2ptr raw__fiber__new(f2ptr cause, f2ptr parent_fiber, f2ptr parent_env, f2ptr cfunkable, f2ptr cfunkable_args) {
  ...
  f2ptr stack_array_index_chunk                           = raw__chunk__new(cause, 8);
  f2ptr iter                                              = nil;
...
def_pcfunk2(fiber__increment_bytes_freed_count, this, bytes_freed_count,
	    "",
	    return f2__fiber__increment_bytes_freed_count(this_cause, this, bytes_freed_count));
...
boolean_t f2__fiber__execute_bytecode(f2ptr cause, f2ptr fiber, f2ptr bytecode) {
  ...
  else if (command == __funk2.bytecode.bytecode__funk_env__symbol)                   {f2__fiber__bytecode__funk_env(                  fiber, bytecode);}
  else if (command == __funk2.bytecode.bytecode__jump_machine_code__symbol)          {f2__fiber__bytecode__jump_machine_code(         fiber, bytecode);}
  else if (command == __funk2.bytecode.bytecode__funk_local_copy__symbol)            {f2__fiber__bytecode__funk_local_copy(           fiber, bytecode);}
...
void raw__fiber__handle_exit_virtual_processor(f2ptr cause, f2ptr this) {
  ...
  }
  {
    ...
    u64 increase_execution_nanoseconds       = end_execution_nanoseconds_since_1970 - start_cycle_execution_nanoseconds;
...
f2ptr raw__fiber__terminal_print_with_frame(f2ptr cause, f2ptr this, f2ptr terminal_print_frame) {
  ...
  if (frame == nil) {
    frame = f2__frame__new(cause, f2list26__new(cause,
						...
						new__symbol(cause, "bytes_freed_count"),          f2__fiber__bytes_freed_count(         cause, this),
...
void f2__fiber__reinitialize_globalvars() {
  ...
  {char* symbol_str = "increment_bytes_freed_count"; __funk2.globalenv.object_type.primobject.primobject_type_fiber.increment_bytes_freed_count__symbol = new__symbol(cause, symbol_str);}
