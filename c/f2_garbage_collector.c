...
void funk2_garbage_collector__handle(funk2_garbage_collector_t* this) {
  if (funk2_processor_mutex__trylock(&(this->do_collection_mutex)) == 0) {
	...
	if (this->gc_pool[index].should_run_gc) {
      ...
      if (should_collect_garbage && (raw__nanoseconds_since_1970() - this->last_garbage_collect_nanoseconds_since_1970) > ((60 * 1000000000ull) / 140)) {
...
void funk2_garbage_collector__save_to_stream(funk2_garbage_collector_t* this, int fd) {
  ...
  {
    f2tid_t* save_gc_pool_thread = (f2tid_t*)from_ptr(f2__malloc(sizeof(f2tid_t) * __funk2.system_processor.processor_count));
    {
      s64 pool_index;
      for (pool_index = 0; pool_index < __funk2.system_processor.processor_count; pool_index ++) {
...
s64 funk2_garbage_collector__load_from_buffer(funk2_garbage_collector_t* this, u8* buffer) {
  u8* buffer_iter = buffer;
  {
    ...
    {
      f2tid_t* load_gc_thread = (f2tid_t*)from_ptr(f2__malloc(sizeof(f2tid_t) * __funk2.system_processor.processor_count));
      {
	s64 pool_index;
	for (pool_index = 0; pool_index < __funk2.system_processor.processor_count; pool_index ++) {
	  load_gc_thread[pool_index] = raw__thread(&funk2_garbage_collector__load_from_buffer__start_thread_load_garbage_collector_pool_buffer, (void*)(&(this->gc_pool[pool_index])));
	}
      }
      {
	s64 pool_index;
	for (pool_index = 0; pool_index < __funk2.system_processor.processor_count; pool_index ++) {
	  raw__join(load_gc_thread[pool_index]);
	}
      }
