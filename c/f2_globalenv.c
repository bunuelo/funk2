...
void funk2_globalenv__reinit(funk2_globalenv_t* this) {
  ...
  this->stdin_stream = f2stream__new(cause, f2integer__new(cause, STDIN_FILENO), nil);
  ...
  this->stdout_stream = f2stream__new(cause, f2integer__new(cause, STDOUT_FILENO), nil);
  ...
  this->stderr_stream = f2stream__new(cause, f2integer__new(cause, STDERR_FILENO), nil);
