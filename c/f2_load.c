...
f2ptr raw__load(f2ptr cause, f2ptr thread, f2ptr filename) {
  ...
  f2ptr load_funk     = f2funk__new(cause, nil, nil, nil, f2cons__new(cause, nil, nil), nil, global_environment(), nil);
  f2ptr load_funk_bcs = f2__compile__funk(cause, environment__lookup_var_value(cause, global_environment(), __funk2.globalenv.trace_all_compiles__symbol), thread, load_funk);
  ...
  while (read_exp != __end_of_file_exception) {
	...
	load_funk     = f2funk__new(cause, nil, nil, nil, f2cons__new(cause, read_exp, nil), read_exp, global_environment(), nil);
	load_funk_bcs = f2__compile__funk(cause,
					  environment__lookup_var_value(cause, global_environment(), __funk2.globalenv.trace_all_compiles__symbol),
					  thread, load_funk);
	...
	if (raw__exceptionp(eval_exp, cause)) {
