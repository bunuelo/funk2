...
void funk2_memorypool__remove_freed_fiber_bytes_freed_counts(funk2_memorypool_t* this) {
  {
    ...
    funk2_hash__key__iteration(&(this->temporary_bytes_freed_count_fiber_hash), creation_fiber,
			       funk2_memblock_t* creation_fiber_memblock = ((funk2_memblock_t*)from_ptr(__f2ptr_to_ptr(creation_fiber)));
...
void funk2_memorypool__free_used_block(funk2_memorypool_t* this, funk2_memblock_t* block) {
  ...
  // remove reference counts
  {
    ...
    case ptype_cons: {
      {
	f2ptr             car       = f2ptr__value(&(((ptype_cons_block_t*)ptype_block)->car));
	...
	funk2_memblock__decrement_reference_count(car_block, car, &(__funk2.garbage_collector));
      ...
      {
	f2ptr             cdr       = f2ptr__value(&(((ptype_cons_block_t*)ptype_block)->cdr));
