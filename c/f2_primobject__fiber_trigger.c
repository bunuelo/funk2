...
def_primobject_1_slot(fiber_trigger, waiting_fiber_set);

f2ptr raw__fiber_trigger__new(f2ptr cause) {
  f2ptr waiting_fiber_set = f2__set__new(cause);
  return f2fiber_trigger__new(cause, waiting_fiber_set);
}
...
f2ptr raw__fiber_trigger_hash__unpause_trigger_fibers(f2ptr cause, f2ptr trigger) {
  ...
  if (fiber_set != nil) {
    set__iteration(cause, fiber_set, fiber,
		   ...
		   f2ptr processor_assignment_index = f2__fiber__processor_assignment_index(cause, fiber);
