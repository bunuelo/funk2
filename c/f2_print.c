...
#define debug_graph 0
	  } else if (f2primobject__is__graph_variable(exp, cause)) {
	    int subexp_size[2];
	    if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
	    {f2__write_pretty__slot_key_and_value("name", 4, cause, fiber, stream, f2graph_variable__name(exp, cause),   f2graph_variable__name__tracing_on(exp, cause), f2graph_variable__name__trace(exp, cause), f2graph_variable__name__imagination_frame(exp, cause),
						  ((recursion_depth == -1) ? recursion_depth : (recursion_depth - 1)), indent_space_num, available_width - width, subexp_size, try_wide, wide_success, show_slot_causes, use_ansi_colors, use_html, brief_mode); width += subexp_size[0]; height += subexp_size[1];}
	  } else if (f2primobject__is__graph_node(exp, cause)) {
	    int subexp_size[2];
#if debug_graph
	    if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
	    {f2__write_pretty__slot_key_and_value("label",                           31, cause, fiber, stream, f2graph_node__label(exp, cause),   f2graph_node__label__tracing_on(exp, cause), f2graph_node__label__trace(exp, cause), f2graph_node__label__imagination_frame(exp, cause),
						  ((recursion_depth == -1) ? recursion_depth : (recursion_depth - 1)), indent_space_num, available_width - width, subexp_size, try_wide, wide_success, show_slot_causes, use_ansi_colors, use_html, brief_mode); width += subexp_size[0]; height += subexp_size[1];}
	    if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
	    {f2__write_pretty__slot_key_and_value("edges_left_node_hash_edge_hash",  31, cause, fiber, stream, f2graph_node__edges_left_node_hash_edge_hash(exp, cause),   f2graph_node__edges_left_node_hash_edge_hash__tracing_on(exp, cause), f2graph_node__edges_left_node_hash_edge_hash__trace(exp, cause), f2graph_node__edges_left_node_hash_edge_hash__imagination_frame(exp, cause),
						  ((recursion_depth == -1) ? recursion_depth : (recursion_depth - 1)), indent_space_num, available_width - width, subexp_size, try_wide, wide_success, show_slot_causes, use_ansi_colors, use_html, brief_mode); width += subexp_size[0]; height += subexp_size[1];}
	    if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
	    {f2__write_pretty__slot_key_and_value("edges_right_node_hash_edge_hash", 31, cause, fiber, stream, f2graph_node__edges_right_node_hash_edge_hash(exp, cause),   f2graph_node__edges_right_node_hash_edge_hash__tracing_on(exp, cause), f2graph_node__edges_right_node_hash_edge_hash__trace(exp, cause), f2graph_node__edges_right_node_hash_edge_hash__imagination_frame(exp, cause),
						  ((recursion_depth == -1) ? recursion_depth : (recursion_depth - 1)), indent_space_num, available_width - width, subexp_size, try_wide, wide_success, show_slot_causes, use_ansi_colors, use_html, brief_mode); width += subexp_size[0]; height += subexp_size[1];}
#else
	    if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
	    ...
	    if (f2graph__root_node(exp, cause)) {
	      if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
	      {f2__write_pretty__slot_key_and_value("root_node",          20, cause, fiber, stream, f2graph__root_node(exp, cause), f2graph__root_node__tracing_on(exp, cause), f2graph__root_node__trace(exp, cause), f2graph__root_node__imagination_frame(exp, cause),
						    ((recursion_depth == -1) ? recursion_depth : (recursion_depth - 1)), indent_space_num, available_width - width, subexp_size, try_wide, wide_success, show_slot_causes, use_ansi_colors, use_html, brief_mode); width += subexp_size[0]; height += subexp_size[1];}
	    }
	    if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
	    ...
	    if (f2graph__root_node(exp, cause)) {
	      max_char_length = 9;
	      if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
	      {f2__write_pretty__slot_key_and_value("root_node", max_char_length, cause, fiber, stream, f2graph__root_node(exp, cause), f2graph__root_node__tracing_on(exp, cause), f2graph__root_node__trace(exp, cause), f2graph__root_node__imagination_frame(exp, cause),
						    ((recursion_depth == -1) ? recursion_depth : (recursion_depth - 1)), indent_space_num, available_width - width, subexp_size, try_wide, wide_success, show_slot_causes, use_ansi_colors, use_html, brief_mode); width += subexp_size[0]; height += subexp_size[1];}
	    }
	    ...
	    {f2__write_pretty__slot_key_and_value("fiber_with_lock",         23, cause, fiber, stream, f2traced_mutex__fiber_with_lock(exp, cause), f2traced_mutex__fiber_with_lock__tracing_on(exp, cause), f2traced_mutex__fiber_with_lock__trace(exp, cause), f2traced_mutex__fiber_with_lock__imagination_frame(exp, cause),
						  ((recursion_depth == -1) ? recursion_depth : (recursion_depth - 1)), indent_space_num, available_width - width, subexp_size, try_wide, wide_success, show_slot_causes, use_ansi_colors, use_html, brief_mode); width += subexp_size[0]; height += subexp_size[1];}
	    if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
	    {f2__write_pretty__slot_key_and_value("lock_stack",              23, cause, fiber, stream, (f2traced_mutex__lock_stack(exp, cause) != nil) ? new__symbol(cause, "<>") : nil, (f2traced_mutex__lock_stack__tracing_on(exp, cause) != nil) ? new__symbol(cause, "<>") : nil, (f2traced_mutex__lock_stack__trace(exp, cause) != nil) ? new__symbol(cause, "<>") : nil, (f2traced_mutex__lock_stack__imagination_frame(exp, cause) != nil) ? new__symbol(cause, "<>") : nil,
						  ((recursion_depth == -1) ? recursion_depth : (recursion_depth - 1)), indent_space_num, available_width - width, subexp_size, try_wide, wide_success, show_slot_causes, use_ansi_colors, use_html, brief_mode); width += subexp_size[0]; height += subexp_size[1];}
	    if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
