...
ansi_color_t print__ansi__string__foreground          = ansi_color__light_purple;
...
ansi_color_t print__ansi__symbol__type__foreground    = ansi_color__light_green;
	  ...
	  if (f2primobject__is__largeinteger(exp, cause)) {
	    ...
	    */
	    ...
	    {f2__write_pretty__slot_key_and_value("leaves",          15, cause, fiber, stream, f2__redblacktree__leaves(cause, exp), nil, nil, nil,
						  ((recursion_depth == -1) ? recursion_depth : (recursion_depth - 1)), indent_space_num, available_width - width, subexp_size, try_wide, wide_success, show_slot_causes, use_ansi_colors, use_html, brief_mode); width += subexp_size[0]; height += subexp_size[1];}
	    if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
	    {f2__write_pretty__slot_key_and_value("value_funk", 15, cause, fiber, stream, f2redblacktree__value_funk(exp, cause), f2redblacktree__value_funk__tracing_on(exp, cause), f2redblacktree__value_funk__trace(exp, cause), f2redblacktree__value_funk__imagination_frame(exp, cause),
						  ((recursion_depth == -1) ? recursion_depth : (recursion_depth - 1)), indent_space_num, available_width - width, subexp_size, try_wide, wide_success, show_slot_causes, use_ansi_colors, use_html, brief_mode); width += subexp_size[0]; height += subexp_size[1];}
	    if (try_wide) {f2__write__space(cause, stream, use_html); width ++;} else {f2__write__line_break(cause, stream, use_html); width = 0; height ++; int i; for (i = 0; i < indent_space_num + width; i++) {f2__write__space(cause, stream, use_html);}}  
