...
f2ptr f2__stream__try_read_number(f2ptr cause, f2ptr stream) {
  ...
  if ((! number_list) || raw__cons__is_type(cause, number_list)) {
    ...
    if (part_decimal_start) {
      ...
      printf("\ndouble=%f", d);
      return f2list3__new(cause, f2integer__new(cause, whole_decimal_value), f2integer__new(cause, part_decimal_value), f2integer__new(cause, part_decimal_denomenator));
