...
f2ptr raw__read(f2ptr cause, f2ptr stream) {
  ...
  //  }
  //}
  if (f2__eq(cause, first_char, __funk2.reader.eof__symbol)) {status("raw_read() note: eof_except."); return __end_of_file_exception;}
  // check all posibilities for first_char
  if (raw__eq(cause, first_char, __char__right_paren))            {return __end_parens_exception;}
  if (raw__eq(cause, first_char, __char__array_right_paren))      {return __array_end_parens_exception;}
  if (raw__eq(cause, first_char, __char__doublelink_right_paren)) {return __doublelink_end_parens_exception;}
  // read list
  ...
  if (raw__eq(cause, first_char, __char__doublelink_left_paren)) {
    ...
    while (1) {
      exp = raw__read(cause, stream);
      if (exp == __doublelink_end_parens_exception) {return seq;} // successfully read end of doublelink list
      if (raw__exception__is_type(cause, exp)) {return exp;} // other exceptions should be propagated
  ...
  if (raw__eq(cause, first_char, __char__escape)) {
    ...
    if (f2__eq(cause, read_ch, __funk2.reader.eof__symbol)) {status("raw_read() note: eof_except."); return __end_of_file_exception;}
    // read hex pointer
	...
	if (f2__eq(cause, read_ch, __funk2.reader.eof__symbol)) {f2__free(to_ptr(str)); status("raw_read() note: eof_except."); return __end_of_file_exception;}
	...
	return __gfunkptr_read__exception;
      }
    ...
    return __illegal_escape_reader_metro_exception;
  }
  // read array
...
void funk2_reader__init(funk2_reader_t* this) {
  ...
  {this->char__escape_char_char       = f2char__new(cause, __escape_char_char);            char* str = "char:escape_char_char";       environment__add_var_value(cause, global_environment(), f2symbol__new(cause, strlen(str), (u8*)str), this->char__escape_char_char);}
...
void funk2_reader__reinit(funk2_reader_t* this) {
  ...
  {char* str = "char:escape_char_char";       this->char__escape_char_char       = environment__safe_lookup_var_value(cause, global_environment(), f2symbol__new(cause, strlen(str), (u8*)str));}
