...
f2ptr raw__exp__terminal_print_with_frame(f2ptr cause, f2ptr this, f2ptr terminal_print_frame) {
  ...
  if (raw__funkable__is_type(cause, funk)) {
	...
	s64   working_size = 0;
	s64   max_size__i  = 1;
	f2ptr max_size     = f2integer__new(cause, max_size__i);
	...
	do {
	  ...
	  if (fits_within_height_constraint != nil) {
	    ...
	    max_size__i  = max_size__i << 1;
	    max_size     = f2integer__new(cause, max_size__i);
	    f2__terminal_print_frame__max_size__set(cause, terminal_print_frame, max_size);
	  }
	} while (working_size != max_size__i);
	if (working_size > 0) {
	...
	// iteratively decrease max size until we satisfy y constraint.
	{
	  s64   working_size                  = 0;
