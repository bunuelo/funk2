...
In SALS, the simulation state graph, $S$, becomes the global
environment.  The global environment is SALS' static representation of
the dynamic activities in Duration.  The previous part described the
simulation state as the mathematical representation for the mind.  I
previously defined this simulation state graph to represent all
...
references.  {\mbox{\autoref{figure:example_global_environment}}}
shows an example global environment with its reference to a frame
object.
\begin{figure}
\center
\includegraphics[width=12cm]{gfx/example_global_environment}
\caption[An example of a global environment.]{An example of a global
  environment with its reference to a frame object.}
