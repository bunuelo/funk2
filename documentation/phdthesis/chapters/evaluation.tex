...
{\mbox{\autoref{table:parallel_processing_speedup}}} shows the speedup
gained by executing multiple processes in parallel on the SALS virtual
machine.
\begin{table}
\centering
\begin{tikzpicture}[y=2cm, x=.15cm,font=\rmfamily]
    \draw (0,0) -- coordinate (x axis mid) (64,0);
    \foreach \y in {0,...,3}
        \draw (0pt,\y) -- (-3pt,\y)
        node[anchor=east] {\y};
    \foreach \y in {2.20}
    ...
    \foreach \x in {0,16,...,64}
        \draw (\x,0pt) -- (\x,-3pt)
        node[anchor=north] {\x};
    \foreach \x in {7}
        \draw (\x,0pt) -- (\x,-3pt)
        node[anchor=north] {\x};
    \node[below=0.8cm] at (x axis mid) {Processes};
    \node[left=0.8cm] at (y axis mid) {Speedup};
    \draw[dashed] (0,2.20) -- (7,2.20);
    \draw[dashed] (7,2.20) -- (7,0);
    \draw plot[mark=*, mark size=1] file {parallel_processing_speedup.data};
\end{tikzpicture}
\caption[The speedup gained by executing multiple processes
  ...
  machine.  Each parallel process in this experiment is performing the
  same numerical processing task on memory that is independent of
  other tasks.  The number of computations performed by a single
  parallel process is used as the unity standard for the speedup
  comparisons.  These tests were run on an Intel Core i7 processor
  with 4 cores, each with 2 hardware hyperthreads.  In these tests, 8
  virtual processors and 8 memory pools were allocated, one for each
  hyperthread in the target hardware platform.  Each test was
  performed 100 times in order to average out variations caused by
  intermittent garbage collections.  The SALS virtual machine performs
  most efficiently on this hardware with 7 parallel fibers executing.}
\label{table:parallel_processing_speedup}
\end{table}
