...
If one is careless with knowledge representations in each subsequently
higher reflective layer, each layer of reflective control will require
exponentially more processing time and space to reason about this
knowledge.  This evaluation will describe how additional reflective
layers in SALS have been carefully designed in order to keep each
additional layer only requiring a linear increase in computational
complexity.
...
knowledge in the layer below.  has the potential to be more complex
and thus more difficult to reason about than the knowledge in the
layers below that knowledge.  For example, a partial state of the
physical problem domain can be a simple relationship such as ``a
pyramid being on a cube.''  Since the deliberative layer contains
meta-physical knowledge, or ``knowledge about physical knowledge,'' a
simple deliberative partial state could be that ``a deliberative plan
fails to accomplish the goal for a cube to be on a pyramid.''  This
deliberative knowledge appears at first glance to be more complex than
the physical knowledge that it is about because it contains, in some
sense, the physical knowledge as well as additional relationships
between this physical knowledge and a plan and a goal.  Further, the
