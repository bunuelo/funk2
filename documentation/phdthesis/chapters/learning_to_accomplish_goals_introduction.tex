...
reflectively learning to accomplish goals.  First, we will develop the
assumption that any process that we choose in our system can be
reflectively monitored and useful information, e.g. beginning and
ending times, will be temporarily stored.  In other words, we assume
that our system natively supports procedural reflection for all
processes.  Given this assumption, we show in this chapter that the
...
Now, we have introduced our problem as being divided into two
processes, the agent and the environment.  Further, we have introduced
how a basic process may be thought of as an \ac{FSM}.  At this point,
we introduce a model for keeping track of the changes in a
computational process: reflective knowledge.  We purposefully make
this assumption before we define the details of the agent model
process because, in our approach, we would like to reflectively reason
about potentially any aspect of this agent process.
