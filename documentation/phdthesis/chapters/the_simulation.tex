...
I will use the symbol {\tt reflective-1} to refer to the activity of
the first-order reflective layer.  This symbol is grounded in the
activity in Duration that currently actually exists.  This symbol does
not refer the simulated activities in Duration in the simulation
model.  This symbol is grounded in the understanding of the model of
mind.  The reflective activity is primarily focusing on the rest of
the simulation state, everything that existed before the
{\tt{reflective-1}} node was added to the simulation state graph.

{\mbox{\autoref{figure:simulation_reflective_1_example_state}}}

\begin{figure}
\includegraphics[width=10cm]{gfx/simulation_reflective_1_example_state}
\caption{The {\tt reflective-1} activity focused on physical activity.}
...
\begin{align}
     ...
     G_E &= {\left\{
               ...
               \end{array}
             \right\}} \\
G_\mu(\overrightarrow{e}) &=
  {\left\{
     \begin{array}{ll}
       \{\text{\tt{being-above}}\} & \text{if }\overrightarrow{e} = (\text{\tt{Gripper-1}}, \text{\tt{Block-1}}) \\
       \text{undefined} & \text{otherwise}
     \end{array}
   \right.}
\end{align}
