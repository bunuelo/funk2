...
activity, $G$, in the simulation state, $S$.
{\mbox{Equations~\ref{equation:graph_representation_in_state_first}}}
{\mbox{through~\ref{equation:graph_representation_in_state_last}}}
show a definition of a graph representation within the state graph.
\begin{align}
\label{equation:graph_representation_in_state_first}
                                         G &\subset S \\
                        x.\text{\tt{node}} &= G_V \\
                    \forall e \in G_E, x_e &\in x.\text{\tt{edge}} \\
   \forall_{e \in G_E}, e = (e_{v_1}, e_{v_2}) \wedge x_e.\text{\tt{tail}} &= \{e_{v_1}\} \\
   \forall_{e \in G_E}, e = (e_{v_1}, e_{v_2}) \wedge x_e.\text{\tt{head}} &= \{e_{v_2}\} \\
\label{equation:graph_representation_in_state_last}
  \forall_{e \in G_E}, x_e.\text{\tt{label}} &= G_\rho(e)
\end{align}

\section{The First-order Reflective Focus}
...
$\text{\tt{reflective}}^1$.  This symbol functions as a representation
of the physical activity that is the current subgraph focus,
$\mathcal{R}^0$, of first-order reflection.
...
define the first-order reflective focus on the given physical
activity.
\begin{align}
\label{equation:first_order_reflection_representation_in_state_first}
                                                                \mathcal{R}^0 &\subset S \\
                                    \text{\tt{reflective}}^1.\text{\tt{node}} &= \mathcal{R}^0_V \\
                    \forall e \in \mathcal{R}^0_E, \text{\tt{reflective}}^1_e &\in \text{\tt{reflective}}^1.\text{\tt{edge}} \\
   \forall_{e \in \mathcal{R}^0_E}, e = (e_{v_1}, e_{v_2}) \wedge \text{\tt{reflective}}^1_e.\text{\tt{tail}} &= \{e_{v_1}\} \\
   \forall_{e \in \mathcal{R}^0_E}, e = (e_{v_1}, e_{v_2}) \wedge \text{\tt{reflective}}^1_e.\text{\tt{head}} &= \{e_{v_2}\} \\
\label{equation:first_order_reflection_representation_in_state_last}
  \forall_{e \in \mathcal{R}^0_E}, \text{\tt{reflective}}^1_e.\text{\tt{label}} &= \mathcal{R}^0_\rho(e)
\end{align}
...
\begin{figure}
\includegraphics[width=12cm]{gfx/reflective_relationship_visualization}
\caption{A reflective relationship visualization.}
...
{\mbox{through~\ref{equation:define_symbol_last}}} define a symbol*
and referent structure:
\begin{align}
\label{equation:define_symbol_first}
                                     X^{*n} &\subset \mathcal{R}^n \\
                                     x^{*n} &\in X^{*n} \\
\label{equation:define_symbol_last}
                            x^{*n}.referent &\subseteq X^{*n} \cup \bigcup_{k=0}^{n-1}\mathcal{R}^k_V \\
\end{align}
Each symbol*, $x^{*n}$, has a referent, $\rho(x^{*n})$, a subgraph of
the simulated activities in Duration, $S$.
{\mbox{Equations~\ref{equation:define_symbol_referent_graph_first}}}
{\mbox{through~\ref{equation:define_symbol_referent_graph_last}}}
define the referent subgraph for a symbol*:
\begin{align}
\label{equation:define_symbol_referent_graph_first}
                               \rho(x^{*n}) &= (\rho_V(x^{*n}), \rho_E(x^{*n}), \rho_\mu(x^{*n})) \\
                             \rho_V(x^{*n}) &= x^{*n}.referent \\
                             \rho_E(x^{*n}) &= (\rho_V(x^{*n}) \times \rho_V(x^{*n})) \cap S_E \\
\label{equation:define_symbol_referent_graph_last}
                        \rho_\mu(x^{*n})(e) &= {\left\{
                                                 \begin{array}{ll}
                                                   S_\mu(e)          & \text{if }e {\in} \rho_E(x^{*n}) \\
                                                   \text{undefined} & \text{otherwise}
                                                 \end{array}
                                               \right.}
\end{align}
