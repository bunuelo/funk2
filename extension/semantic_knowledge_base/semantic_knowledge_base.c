	      ...
	      semantic_frame__iteration(cause, semantic_frame, key_type, key, value,
					...
					}
					{
					  ...
					  if (! raw__ptypehash__contains(cause, edge_code_ptypehash, relationship)) {
					    status("debug 19");
					    f2ptr edge_label = f2__relationship_meta_semantic_object__label(cause, relationship);
					    if (raw__larva__is_type(cause, edge_label)) {
					      return edge_label;
					    }
					    status("debug 19.5");
					    f2ptr edge_code = f2__graphviz__raw_labelled_edge(cause, edge_label, f2__graphviz__exp__as__name(cause, semantic_frame), f2__graphviz__exp__as__name(cause, value));
