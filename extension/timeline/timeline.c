...
f2ptr raw__timeline__cairo_render(f2ptr cause, f2ptr this, f2ptr cairo_context) {
  ...
  
  {
    ...
    raw__cairo_context__scale(cause, cairo_context, (1.0 / timeline__x_width), (1.0 / timeline__x_width));
    // box
	...
	for (x = 0; x < (((int)(timeline__x_width + 0.5)) - 1); x ++) {
	  raw__cairo_context__arc(cause, cairo_context, x + 1.0, y + 1.0, 0.001, 0, 2 * cairo_pi);
