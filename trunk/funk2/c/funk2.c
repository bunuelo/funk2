...
void f2__initialize() {
  ...
  pause_gc(); f2__primobject__time__initialize();            resume_gc(); try_gc(); release__assert(! gc__is_disabled(), nil, "garbage collection should be enabled at this point.");
...
void funk2__init(funk2_t* this, int argc, char** argv) {
  ...
  if (! raw__funkablep(boot_funk, cause)) {
	...
	load_funk = environment__lookup_funkvar_value(cause, global_environment(), f2symbol__new(cause, strlen("primfunk:load"), (u8*)"primfunk:load"));
      ...
      if (raw__funkablep(repl_funk, cause)) {
